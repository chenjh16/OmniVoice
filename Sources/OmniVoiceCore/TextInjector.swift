...
public final class TextInjector {
    ...
    private func focusedElementResult(targetApp: NSRunningApplication?) -> (element: AXUIElement?, error: AXError) {
        if let targetApp {
            ...
            if let element = copyAXElement(appElement, attribute: kAXFocusedUIElementAttribute as CFString) {
                let role = stringAttribute(element, kAXRoleAttribute as CFString)
        ...
        guard let element = axElement(from: focusedValue) else { return (nil, .failure) }
        AXUIElementSetMessagingTimeout(element, 0.35)
        return (element, .success)
    }
    ...
    private func attemptAccessibilityInsertion(text: String, element: AXUIElement, focus: FocusSnapshot) async -> Bool {
        ...
        if traits.selectedTextSettable {
            let beforeValue = stringAttribute(element, kAXValueAttribute as CFString)
            let selectedRange = selectedTextRange(element: element)
        ...
        guard traits.valueSettable,
              let currentValue = stringAttribute(element, kAXValueAttribute as CFString),
    ...
    private func verifyAXWrite(
        ...
        let afterValue = stringAttribute(element, kAXValueAttribute as CFString)
    ...
    private func selectedTextRange(element: AXUIElement) -> NSRange? {
        var value: CFTypeRef?
    ...
    private func setSelectedTextRange(element: AXUIElement, location: Int) {
    ...
    private func copyAXElement(_ element: AXUIElement, attribute: CFString) -> AXUIElement? {
        ...
        guard AXUIElementCopyAttributeValue(element, attribute, &value) == .success,
              let copied = axElement(from: value) else {
            return nil
        }
        AXUIElementSetMessagingTimeout(copied, 0.35)
        return copied
    }

    private func copyAXElementArray(_ element: AXUIElement, attribute: CFString) -> [AXUIElement] {
        var value: CFTypeRef?
        guard AXUIElementCopyAttributeValue(element, attribute, &value) == .success,
    ...
    private func firstEditableDescendant(in root: AXUIElement) -> AXUIElement? {
        ...
        while !queue.isEmpty, visited < 300 {
            ...
            for attribute in preferredAttributes {
                if let single = copyAXElement(element, attribute: attribute) {
                    queue.append(single)
                } else {
                    queue.append(contentsOf: copyAXElementArray(element, attribute: attribute))
                }
            }
        }
        return nil
