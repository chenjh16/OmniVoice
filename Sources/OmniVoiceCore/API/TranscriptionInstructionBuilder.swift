...
public enum TranscriptionInstructionBuilder {
    public static func instruction(style: TranscriptionStyle) -> String {
        ...
        case .verbatim:
            return """
            请把这段音频转换为适合直接输入的最终文本。默认输出简体中文；如果音频主要是英文，则输出英文；如果音频中中英文混杂，请保留原始语境中的中英文混杂内容。尽量保留用户原始说法，只做必要的语音识别纠错和自然标点补全，不主动删减口水词、重复表达或停顿痕迹。保留用户明确表达的语气、事实、顺序、技术词、代码标识符、产品名、文件名、命令和专有名词。只输出最终文本，不要解释、不要对话、不要 Markdown。
            """
        case .codeFaithful:
            return """
