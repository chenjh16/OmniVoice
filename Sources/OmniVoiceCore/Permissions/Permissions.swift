...
public enum PermissionChecker {
    ...
    public static func speechRecognitionGranted() -> Bool {
    ...
    private static func openSettings(_ urlString: String) {
        ...
        for delay in SystemSettingsActivationPlanner.activationDelays {
    ...
    private static func activateSystemSettings() {
        NSRunningApplication
            .runningApplications(withBundleIdentifier: SystemSettingsActivationPlanner.bundleIdentifier)
            .first?
            .activate()
    }
}

public enum SystemSettingsActivationPlanner {
    public static let bundleIdentifier = "com.apple.systempreferences"
    public static let activationDelays: [TimeInterval] = [0.2, 0.8]
}
...
public enum PermissionRequestFollowUpPlanner {
    public static func shouldAttemptListeningRestart(
        openedSettingsFallback: Bool,
        autoEnabled: Bool,
        listeningEnabled: Bool,
        globalStopActive: Bool
    ) -> Bool {
        !openedSettingsFallback &&
        !autoEnabled &&
        listeningEnabled &&
        !globalStopActive
    }

    public static func shouldContinueGuideAfterAccessibilityFallback(
        openedSettingsFallback: Bool,
        isManualRequest: Bool
    ) -> Bool {
        !(openedSettingsFallback && isManualRequest)
    }
}
