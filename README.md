...
OmniVoice 只从 `~/.config/omnivoice/config.jsonc` 读取连接配置和菜单偏好。这个文件使用 JSONC，支持注释和尾随逗号。环境变量和 Keychain 都不是 App 配置来源。

请通过 `配置` 子菜单里的 `打开 config 文件` 编辑连接设置。如果文件不存在、格式错误或缺少必要字段，OmniVoice 会先把旧文件备份为 `config.jsonc.bak-YYYYMMDD-HHMMSS`，再创建当前界面语言的完整 JSONC 模板，然后优先用 VS Code、Cursor、Windsurf、Zed、Sublime Text、BBEdit、TextMate 或 Xcode 等代码编辑器打开；如果都找不到，则回退到系统默认应用。API Key 只在 `config.jsonc` 中编辑，菜单中始终脱敏展示。

菜单里的模型、语言、风格、关键词提示开关、关键词组多选、触发键、录音时长、自动插入、开机自启、HUD 样式、提示显示时长、HUD 弹出延迟、API 来源和测速频率都会写回 `config.jsonc`。切换界面语言时，OmniVoice 也会用对应语言重写配置注释。
...
```jsonc
{
  ...
  "preferences": {
    ...
    "enabled_keyword_groups": ["omnivoice_terms", "technical_terms"],
...
可以用 `keyword_groups` 添加多组关键词，再通过菜单里的 `关键词` 总开关和分组勾选控制是否注入。关键词只作为识别消歧提示使用：模型会在发音和上下文合理时优先参考它们，但不会因为列表里有某个词就强行输出。每组最多使用 200 个关键词，总计最多注入 500 个。`display_name`、`display_name_zh`、`description` 和 `description_zh` 不需要双语都写；缺失的一种界面语言会自动回退到已有文本。
...
菜单按控制、配置、转写、体验/系统和最后 App 操作分段组织，包含 `停止` 或 `重新启用`、状态、权限、脱敏 Base URL/API Key/当前模型/来源、`配置`、`语言`、`风格`、`关键词`、`触发键`、`录音时长`、`开机自启`、`自动插入`、`提示与显示`、`权限管理`、`重启 OmniVoice` 和 `退出 OmniVoice`。

`提示与显示` 子菜单包含 `HUD 样式`、`提示显示时长` 和 `HUD 弹出延迟`。HUD 弹出延迟默认 200ms，可选 100ms、200ms、300ms、400ms、500ms；录音仍会在按下触发键时立即开始。
...
OmniVoice stores connection settings and menu preferences only in `~/.config/omnivoice/config.jsonc`. The file uses JSONC, so comments and trailing commas are allowed. Environment variables and Keychain are not app configuration sources.

Use `Open config file` from the `Configuration` submenu to edit connection settings. If the file is missing, invalid, or incomplete, OmniVoice first backs up the old file as `config.jsonc.bak-YYYYMMDD-HHMMSS`, then creates a complete localized JSONC template and tries to open it in an installed code editor such as VS Code, Cursor, Windsurf, Zed, Sublime Text, BBEdit, TextMate, or Xcode. If none is available, it falls back to the system default app. API keys are edited only in `config.jsonc` and are always redacted in the menu.

Menu choices for model, language, style, keyword hint enablement, keyword group selection, trigger key, recording duration, Auto Insert, Launch at Login, HUD style, message duration, HUD reveal delay, API source, and latency interval are written back to `config.jsonc`. When you switch UI language, OmniVoice rewrites the config comments in the selected language.
...
Use `keyword_groups` to add multiple keyword groups, then use the `Keyword Hints` menu to enable the feature and multi-select groups. Keywords are recognition hints only: the model should prefer them when the audio and context fit, but should not force a listed word into the result. Each group uses at most 200 keywords, and at most 500 keywords are injected in total. `display_name`, `display_name_zh`, `description`, and `description_zh` do not all need to be present; the missing UI language falls back to whichever text exists.
...
The menu is grouped as control, configuration, transcription, experience/system, and final app actions. It includes `Stop` or `Re-enable`, status, permissions, redacted Base URL/API Key/current model/source, `Configuration`, `Language`, `Transcription Style`, `Keyword Hints`, `Trigger Key`, `Recording Duration`, `Launch at Login`, `Auto Insert`, `Display & Hints`, `Permission Management`, `Restart OmniVoice`, and `Quit OmniVoice`.

`Display & Hints` contains `HUD Style`, `Message Duration`, and `HUD Reveal Delay`. HUD Reveal Delay defaults to 200ms and offers 100ms, 200ms, 300ms, 400ms, and 500ms; recording still starts immediately when the trigger is pressed.
